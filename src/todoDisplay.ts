...
export const addTodoHandler = (todoList: Todo[], content: unknown, dueDate: unknown, todoListDisplay: HTMLUListElement) => {
  addTodo(todoList, content, dueDate);
  renderTodoList(todoList, todoListDisplay);
}

const removeTodoHandler = (todoList: Todo[], id: string, todoListDisplay: HTMLUListElement) => {
  removeTodo(todoList, id);
  renderTodoList(todoList, todoListDisplay);
}

const toggleTodoHandler = (todoList: Todo[], id: string, todoListDisplay: HTMLUListElement) => {
  toggleTodo(todoList, id);
  renderTodoList(todoList, todoListDisplay);
}

export const renderTodoList = (todoList: Todo[], todoListDisplay: HTMLUListElement) => {
